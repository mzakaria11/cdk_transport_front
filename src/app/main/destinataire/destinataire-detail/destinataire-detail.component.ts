...
export class DestinataireDetailComponent implements OnInit, OnDestroy {
            ...
            swalWithBootstrapButtons.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    let body: any = {
                        ...
                        "idPays": idPays,
                        "isAffretement": form.value.isAffretement,
                        "isMessagerie": form.value.isMessagerie,
                    };

                    if (form.value.isDelta) {
                        body.delta = form.value.delta;
                    }
    ...
    ngOnInit(): void {
        ...
        this._destinataireDetailService.onDestinataireDetailChanged
            ...
            .subscribe(response => {
                this.data = response;
                if (this.data.delta == null){
                    this.data.isDelta = false;
        ...
        this.destinataireForm?.controls?.isAffretement.valueChanges.subscribe(value => {
            if (value) {
                this.destinataireForm?.controls?.isMessagerie.setValue(false);
            }
        });

        this.destinataireForm?.controls?.isMessagerie.valueChanges.subscribe(value => {
            if (value) {
                this.destinataireForm?.controls?.isAffretement.setValue(false);
            }
        });

        // Set up the behavior for the isDelta checkbox
        this.destinataireForm?.controls?.isDelta.valueChanges.subscribe(value => {
            if (value) {
                this.destinataireForm?.controls?.delta.setValidators([Validators.required]);
            } else {
                this.destinataireForm?.controls?.delta.clearValidators();
            }
            this.destinataireForm?.controls?.delta.updateValueAndValidity();
        });
    }
